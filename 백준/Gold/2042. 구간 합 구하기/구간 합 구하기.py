...
class SegmentTree:
    ...
    def update_value(self, order, value):
        diff = value-self.nums[order-1]
        self.nums[order-1] = value
        self.update_tree(diff, order)
        
    def update_tree(self, diff, order, start=1, end=-1, idx=1):
        if end == -1:
            end = self.end
        if start <= order <= end:
            if start == end:
                ...
                mid = (start+end)//2
                self.tree[idx] += diff
                self.update_tree(diff, order, start, mid, idx*2)
                self.update_tree(diff, order, mid+1, end, idx*2+1)

n, m, k = map(int, stdin.readline().split())
nums = [int(stdin.readline()) for _ in range(n)]
tree = SegmentTree(nums, lambda a, b:a+b)
for _ in range(m+k):
